...
- Experimental 1Password integration via the `qute-1pass-sidecar` daemon.
  Commands: `:onepassword-{pick,fill,fill-field,copy-username,copy-password,copy-totp,save,status,restart-sidecar}`.
  Secrets copied to the clipboard (password, TOTP) are automatically cleared after 30 seconds.
  `:onepassword-pick` opens a completion popup with URL-matched vault items.
  `:onepassword-fill-field` fills the focused element only — useful for multi-step login forms.
  `onepassword.*` settings. Passkey support is plumbing-ready but
  capability-gated; only the experimental `native` backend (off by default)
  will exercise it. See `doc/onepassword.asciidoc` for setup instructions.
- Experimental `native` backend for the 1Password sidecar: reverse-engineers
  the Chrome Native Messaging channel of `/opt/1Password/1Password-BrowserSupport`
  to validate vault state without requiring the `op` CLI. Requires the bundled
  `qute-1pass-sidecar` bridge launcher installed at `/usr/local/bin/` (root-owned)
  and registered in `/etc/1password/custom_allowed_browsers`. Falls back
  transparently to `op-cli` on startup failure; the degraded state and reason are
  surfaced via `:onepassword-status`. Gated behind
  `onepassword.experimental_bridge = true` as this likely violates 1Password's
  Terms of Service.
- FIDO2 User Verification support for WebAuthn dialogs (#8533).

Fixed
~~~~~

- `:onepassword-fill` now opens the picker when more than one vault item
  matches the page URL, instead of silently picking the first match.
  Fixes wrong credentials being filled when the same site has logins stored
  in multiple vaults. The picker now also shows a "Vault" column for
  explicit disambiguation.
- `:onepassword-fill` URL matching now uses strict subdomain rules:
  `evilgithub.com` no longer matches a `github.com` vault item.
- URL matching in the 1Password integration now uses the Mozilla Public
  Suffix List (via `publicsuffixlist`) to compute the registrable domain.
  The previous "last two dot-separated parts" heuristic produced false
  matches on multi-level ccTLDs (`bar.co.uk` ↔ `baz.co.uk`) and on shared
  public hosts (`*.herokuapp.com`, `*.github.io`, `*.appspot.com`) — a
  credential-theft vector when an attacker controls a sibling subdomain on
