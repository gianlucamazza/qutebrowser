...
WebAuthn/FIDO2 UX requests
^^^^^^^^^^^^^^^^^^^^^^^^^^

With Qt 6.7 or newer, QtWebEngine exposes WebAuthn user verification requests
via https://doc.qt.io/qt-6/qwebenginepage.html#webAuthUxRequested[`QWebEnginePage.webAuthUxRequested`]
and https://doc.qt.io/qt-6/qwebenginewebauthuxrequest.html[`QWebEngineWebAuthUxRequest`].
qutebrowser handles those requests in QtWebEngine-specific code, as this API
does not exist for QtWebKit.

Keep this layer limited to browser/WebAuthn UX state handling: PIN collection,
resident credential account selection, token touch prompts, cancellation,
completion and request failure reporting. A request can be cancelled while a
qutebrowser prompt is still open, so code must not keep using a stale
`QWebEngineWebAuthUxRequest` after returning from a prompt.

Password-manager passkey integrations are separate from this browser UX layer
and must not touch `_WebEngineWebAuth`. An external bridge (e.g. via
`QWebChannel` + a sidecar process) that intercepts `navigator.credentials.*`
in the page's JavaScript world is acceptable provided it:

* is capability-gated (only activated when the backend advertises support),
* falls back cleanly to the Qt WebAuthn UX above when the capability is absent,
* is explicitly opt-in via a config key (e.g. `onepassword.experimental_bridge`),
* and documents any Terms-of-Service implications to the user.

Do **not** modify `_WebEngineWebAuth` to dispatch passkey operations — that
class must remain limited to pure UX state (PIN, account selection, touch).
